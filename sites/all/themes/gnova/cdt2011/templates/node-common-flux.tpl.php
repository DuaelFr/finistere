...
<?php //drupal_set_message('<pre>'.print_r($node->type,true).'</pre>'); ?>
        ...
        if( false && $field_fl_tarif_min[0]['view'] && $field_fl_adherent_cdt_tx[0]['value'] == 'oui') { ?>
            ...
            if ($field_fl_adherent_cdt_tx[0]['value'] == 'oui') {
              ...
              if ($field_fl_visite_virtuelle_tx[0]['value'] && $field_fl_visite_virtuelle_tx[0]['value'] != NULL) { 
                ...
                print '<div class="virtual-visit">'. l(t('Virtual visit'), $field_fl_visite_virtuelle_tx[0]['value'], array('html' => TRUE, 'attributes' => array('id' => 'virtual'))) .'</div>';
              }
            }
            /*
            else {
              print '<div class="address"><ul><li>'.$node->locations[0]['street'].'</li>';
              print '<li>'.$node->locations[0]['postal_code'] .' '. $node->locations[0]['city'].'</li>
              </ul></div>';
            }
            */
            ?>
			...
			<div class="uboxpb_items_left">
				<div class="uboxbp_content">
					...
					<?php if(($node->field_fixfl_onlineresa[0]['value']=='oui' || $node->field_fixfl_onlineresa[0]['value']=='yes') && 
