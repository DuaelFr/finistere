<div class="box_agenda clearfix file-item">
  ...
  <div class="content_box_agenda">
    ...
    $type = '';
    // Hotel
    if (in_array(152, $taxo) || in_array(2621, $taxo)) {
      $type = 'z3132e9_fr';
    }
    // Hotel EN
    if (in_array(2622, $taxo)) {
      $type = 'z3132e9_uk';
    }
    // Camping
    if (in_array(410, $taxo)) {
      $type = 'z3130e9_fr';
    }
    // Camping EN
    if (in_array(2704, $taxo)) {
      $type = 'z3130e9_uk';
    }
    // Chambres meublés
    if (in_array(2629, $taxo)) {
      $type = 'z2852e3_fr';
    }
    // Chambres meublés EN
    if (in_array(2701, $taxo)) {
      $type = 'z2852e3_uk';
    }
    // Chambres d'hôte
    if (in_array(2402, $taxo)) {
      $type = 'z3131e9_fr';
    }
    // Chambres d'hôte
    if (in_array(2623, $taxo)) {
      $type = 'z3131e9_uk';
    }
    ...
    if ($type != ''):
      if (($fields['field_fixfl_onlineresa_value']->content == 'oui' || $fields['field_fixfl_onlineresa_value']->content == 'yes') &&
        ...
        <div class="wrap_btn clearfix">
          <a class="rsv_btn" title="Réserver en ligne"
